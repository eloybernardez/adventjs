...
function getMaxGifts(giftsCities, maxGifts, maxCities) {
  // Create subsets with gifts for each city
  ...
  // first get amount of cities and then sum gifts to be delivered
  const maxGiftsForCity = getAllSubsets(giftsCities)
    .filter((city) => city.length <= maxCities)
    .map((city) => city.reduce((acc, city) => acc + city, 0));

  // return the max number of gifts to be delivered
  return Math.max(...maxGiftsForCity.filter((city) => city <= maxGifts));
}

console.group("Max delivarable gifts");
console.log(getMaxGifts([12, 3, 11, 5, 7], 20, 3)), // 20
  ...
  console.log(getMaxGifts([50, 10, 40, 1000, 500, 200], 200, 4)), // 200
  console.log(getMaxGifts([50, 10, 40, 1, 500, 200], 76, 3)), // 61
  console.groupEnd();
